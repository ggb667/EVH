...
@trace_calls
def iter_all_chart_sources(cache: dict[str, Any]) -> Iterable[tuple[dict[str, Any], dict[str, Any], dict[str, Any]]]:
    for account in cache.get("accounts") or []:
        ...
        for patient in cache.get("patients_by_account", {}).get(client_id, []):
            ...
            for chart in _chart_files_for_patient(patient_id):
                yield account, patient, chart
...
def main(argv: list[str] | None = None) -> int:
    ...
    while True:
        ...
        for account, patient, chart in iter_all_chart_sources(discovery_cache):
            ...
            client_name = _text(account.get("name") or account.get("displayName") or account.get("businessName")) or client_id
            patient_name = _text(patient.get("name") or patient.get("patientName")) or patient_id
            if not client_id or not patient_id or not pdf_id:
                continue
            if args.limit_clients and len(seen_clients) >= args.limit_clients:
                print(json.dumps({"status": "loop_break", "reason": "limit_clients", "clients_seen": len(seen_clients), "patients_seen": len(seen_patients), "pdfs_seen": pdf_count}, indent=2, sort_keys=True), flush=True)
                break
            if args.limit_patients and len(seen_patients) >= args.limit_patients:
                print(json.dumps({"status": "loop_break", "reason": "limit_patients", "clients_seen": len(seen_clients), "patients_seen": len(seen_patients), "pdfs_seen": pdf_count}, indent=2, sort_keys=True), flush=True)
                break
            if args.limit_pdfs and pdf_count >= args.limit_pdfs:
                print(json.dumps({"status": "loop_break", "reason": "limit_pdfs", "clients_seen": len(seen_clients), "patients_seen": len(seen_patients), "pdfs_seen": pdf_count}, indent=2, sort_keys=True), flush=True)
                break
            ...
            if not resume_reached:
                if (client_id, patient_id, pdf_id) == resume_cursor:
                    resume_reached = True
                else:
                    continue
            ...
            pdf_count += 1
            progress_state["clients_seen"] = len(seen_clients)
            progress_state["patients_seen"] = len(seen_patients)
