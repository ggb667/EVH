...
init_runtime_paths() {
  ...
  export AGENIC_PROJECT_PONY_RUNTIME_INSTALL_NOTICE_PATH="$AGENIC_PROJECT_PONY_RUNTIME_DIR/install.notice"
...
ensure_runtime_layout() {
  ...
  [[ -f "$AGENIC_PROJECT_PONY_RUNTIME_INSTALL_NOTICE_PATH" ]] || : >"$AGENIC_PROJECT_PONY_RUNTIME_INSTALL_NOTICE_PATH"
...
cmd_pending_notice() {
  ...
  if [[ -s "$AGENIC_PROJECT_PONY_RUNTIME_INSTALL_NOTICE_PATH" ]]; then
    cat "$AGENIC_PROJECT_PONY_RUNTIME_INSTALL_NOTICE_PATH" >"$AGENIC_PROJECT_PONY_RUNTIME_PENDING_NOTICE_PATH"
  fi
  local item_id
  while IFS= read -r item_id; do
    [[ -n "$item_id" ]] || continue
    ...
    if [[ "$SOURCE" == "agent" ]]; then
      ...
      show_notice_for_item "$item_id" >>"$AGENIC_PROJECT_PONY_RUNTIME_PENDING_NOTICE_PATH"
      return 0
    fi
  done < <(queue_ids)
}
