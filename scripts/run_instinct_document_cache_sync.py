...
from scripts.instinct_cache_sync_pipeline import connect_db, emit, sync_documents
from scripts.instinct_identity_sync import InstinctApiSyncClient


def main() -> int:
    parser = argparse.ArgumentParser(description="Step 1.3: sync Instinct documents into rag_source_document")
    ...
    parser.add_argument("--log-file", default="etl_logs/instinct_step1_3_documents.log")
    ...
    with log_path.open("a", encoding="utf-8") as logf:
        ...
        emit(log, "step_start", step="1.3", log_file=str(log_path), patient_limit=args.patient_limit)
        client = InstinctApiSyncClient(args.base_url, args.client_id, args.client_secret)
        with connect_db() as conn:
            summary = sync_documents(client, conn, log=log, patient_limit=args.patient_limit)
        emit(log, "step_done", step="1.3", **summary.__dict__)
    return 0
